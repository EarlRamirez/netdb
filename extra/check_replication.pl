...
my $db_host = "remote_db_host";
my $db_port = "";
my $db_sock = "";
my $db_user = "root";
my $db_pass = "remote_db_passwd";
my $db_name = "";
my $optstatus;
...
&nagios_return("CRITICAL", "Replication problem: Seconds_Behind_Master=".$result->{'Seconds_Behind_Master'}) if ($result->{'Seconds_Behind_Master'} > 2000);
&nagios_return("WARNING",  "Replication problem: Seconds_Behind_Master=".$result->{'Seconds_Behind_Master'}) if ($result->{'Seconds_Behind_Master'} > 1000);
$sth->finish();
