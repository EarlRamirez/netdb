#!/usr/bin/perl
###########################################################################
# fortigatescraper.pl - Fortigate Scraper Plugin
# Author: Jonathan Yantis <yantisj@gmail.com>
# Copyright (C) 2014 Jonathan Yantis
###########################################################################
# 
# Supports Fortigate Firewall ARP Table Scraping Only
#
# fortigatescraper.pl -d switch.domain.com[,arp,vrf-dmz,forcessh] \
...
sub connectDevice {
	...
	eval {
	    ...
	    $session = get_SSH_session( $$devref{fqdn}, "terminal length 0", $devref );
	    ...
	    ## Print Sample Output
            #@output = SSHCommand( $session, "show version" );
	    #print "DEBUG: sh ver output: @output\n\n" if $DEBUG;
...
sub getMacTable {
	...
	if ( $line =~ /(\w\w\:){5}|(\w\w\w\w\.\w\w\w\w\.\w\w\w\w)/ ) {
		...
		print "debug: acceptable data: $$devref{host},$mac[1],$port,,$mac[0]\n" if $DEBUG>2;		
		push( @mactable, "$$devref{host},$mac[1],$port,,$mac[0]" );
	    }
...
sub getInterfaceTable {
    ...
    # Te 0/0   Malawi VNic  Up     10000 Mbit Full   1-2,4,10,12,15,25-26,101
    # Te 0/1   Malawi Vnic  Up     10000 Mbit Full   1-2,4,10,12,15,25-26,101
    # Te 0/2                Down   Auto      Auto   1
    # Te 0/3                Up     1000 Mbit Full   4,10,100
    # Te 0/4                Down   Auto      Auto   1
    ...
    my @cmdresults = SSHCommand( $session, "show interface status" );
	...
	if ( $line =~ /\s(Up|Down)\s/ ) {
	    ...
	    my @data = split( /\s\s(Up|Down)\s\s+/, $line );
	    
	    my @front = split( /\s+/, $data[0] );
	    $port = "$front[0]$front[1]";
	    shift @front;
	    shift @front;
	    $desc = "@front";

	    my @back = split( /\s+/, $data[1] );
	    ...
	    print "Accepted int status line: $$devref{host},$port,$state,$vlan,$desc,$speed,$duplex\n" if $DEBUG>5;
	    push( @intstatus, "$$devref{host},$port,$state,$vlan,$desc,$speed,$duplex" );
	}
	else {
	    print "Debug ($DEBUG): Unmatched int status data: $line\n" if $DEBUG>2;
	}
    }	
...
sub getARPTable {
    ...
    my @cmdresults = SSHCommand( $session, "diagnose ip arp list" );
	...
	if ( $line =~ /ifname/ ) {
	    ...
	    # Look for IP in field 2
	    if ( $data[2] =~ /[0-9]{1,3}(\.[0-9]{1,3}){3}/ ) {
		push( @arptable, "$data[2],$data[3],,$port[1]" );
	    }
	    else {
		print "DEBUG: Discarded ARP Table Data: $line\n" if $DEBUG>3;
	    }
	}
	else {
	    print "DEBUG: Discarded ARP Table Data: $line\n" if $DEBUG>3;
...
sub usage {
    print <<USAGE;
    Usage: fortigatescraper.pl [options] 
