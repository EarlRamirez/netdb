#!/usr/bin/perl
###########################################################################
# foundryscraper.pl - Foundry Scraper Plugin
...
#  v1.? - 2013-10-14 - general function to handle paging
#
###########################################################################
# License:
#
...
my $foundry = connectDevice( $devref );
...
sub connectDevice {
    ...
    ( $session,$type ) = getSessionFoundry( $devref );
    if ($type =~ /ssh/) {
        $ssh_session = 1;
    }
    else{
...
sub getMacTable {
    ...
    eval {
        # SSH Command
        if ( $ssh_session ) { 
            #@cmdresults = SSHCommand( $session, "show mac-address", $ssh_cmd_timeout );
...
########################################
#---------------------------------------------------------------------------------------------
# Get the full interface descriptions and port status information on Brocade (foundry) device
...
sub getSessionFoundry {
    ...
    # Fallback to Telnet if allowed
        ...
        $EVAL_ERROR = undef;
        eval {
            $session = get_foundry_session_auto( $fqdn );
