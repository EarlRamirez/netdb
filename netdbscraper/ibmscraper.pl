#!/usr/bin/perl
###########################################################################
# ibmscraper.pl - Skeleton Scraper Plugin
# Author: Jonathan Yantis <yantisj@gmail.com>
# Copyright (C) 2014 Jonathan Yantis
###########################################################################
# 
# IBM Blade Scraper, experimental support
#
## Device Option Hash:
...
sub connectDevice {
	...
	eval {
	    ...
	    $session = get_SSH_session( $$devref{fqdn}, "no paging command", $devref );
	    ...
	    my @output;
            ...
            # Turn off paging
            @output = SSHCommand( $session, "terminal-length 0" );
...
sub getMacTable {
	...
	if ( $line =~ /(\w\w\:){5}|(\w\w\w\w\.\w\w\w\w\.\w\w\w\w)/ ) {
		...
		print "debug: acceptable data: $$devref{host},$mac[1],$mac[3],,$mac[2]\n" if $DEBUG>3;		
		push( @mactable, "$$devref{host},$mac[1],$mac[3],,$mac[2]" );
	    }
...
sub getInterfaceTable {
    ...
    # Capture intstatus
    @output1 = SSHCommand( $session, "show int status" );
    # Results returned in one scalar, split out
    @output1 = split( /\r/, $output1[0] );

    # Capture int inf
    @output2 = SSHCommand( $session, "show int inf" );
    # Results returned in one scalar, split out
    @output2 = split( /\r/, $output2[0] );

    # NetDB Interface Status Data: (switch,port,status,vlan,description,speed,duplex)
    # mdcg27bcsw1,Gi1/0/20,connected,trunk,[mdcg26sw1],a-1000,a-full

    # Run show int inf first to get the native vlan id
	...
	#------- ---- --- ---------- ---- --- --- -------- ------ -------------- ------------------------
	#1       1     y  External    d    d   e      d       1          1 512 516 519 530 535 536 537 
	
	# If it's a trunk, put trunk in the vlan field, otherwise get the PVID
	if ( $inf[8] && $line =~ /^\w+/ && $inf[1] =~ /\d+/ ) {
	    if ( $inf[2] eq 'y' ) {
		$pvid{$inf[0]} = "trunk";
	    }
	    else {
	    ...
	    print "Discarded sh int inf data: $line\n" if $DEBUG>2;
	}
    }

    # Run show int status to get the other status info
	...
	if ( $status[1] =~ /\d+/ && $status[6] ) {
	    
	    # Split on status and get the description
	    my( $tmp, $desc ) = split(/\s\s+$status[6]\s\s+/, $line );

	    $status[6] = "connected" if $status[6] =~ "up";
	    $status[6] = "notconnect" if $status[6] =~ "down";
	    $status[6] = "err-disabled" if $status[6] =~ "errdisabled";

	    print "Accepted: $$devref{host},$status[0],$status[6],$pvid{$status[0]},$desc,$status[2],$status[3]\n" if $DEBUG>2;
	    push( @intstatus, "$$devref{host},$status[0],$status[6],$pvid{$status[0]},$desc,$status[2],$status[3]" );
	}
	else {
	    print "Discarded sh int status data: $line\n" if $DEBUG>2;
	}
    }
