#!/usr/bin/perl
###########################################################################
# dellscraper.pl - Dell Scraper Plugin
# Author: Jonathan Yantis <yantisj@gmail.com>
# Copyright (C) 2013 Jonathan Yantis
###########################################################################
# 
# Dell Powerconnect Scraper (expects enable mode default)
#
# Experimental.
#
...
# this:
#
# dellscraper.pl -d switch.domain.com[,arp,vrf-dmz,forcessh] \
...
# Expect Login As: prompt
#my $login_prompt = "Login as:";
my $login_prompt = "User Name:";
...
sub connectDevice {
	...
	eval {
	    ...
	    # Get a new SSH session object
            ...
            my @output = $session->login("$login_prompt","Password:");
            ...
            # Turn off paging
            @output = SSHCommand( $session, "terminal datadump" );
	...
	die "This scraper does not support telnet";
    }
}
...
sub getMacTable {
    ...
    @output = SSHCommand( $session, "show bridge address-table" );
	...
	if ( $line =~ /(\w\w\:){5}|(\w\w\w\w\.\w\w\w\w\.\w\w\w\w)/ ) {
		...
		# Save VLAN for port for intstatus
		$vlan{$mac[2]} = $mac[0];
...
sub getInterfaceTable {
    ...
    # Capture mac table from device
    my @output = SSHCommand( $session, "show interfaces status" );
	...
	if ( $int[0] =~ /\d+/
	    ...
	    $int[6] = "connected" if $int[6] eq 'Up';
	    $int[6] = "notconnect" if $int[6] eq 'Down';
	    $int[6] = "notpresent" if $int[6] eq 'Not';

	    print "debug: acceptable data: $$devref{host},$int[0],$int[6],$vlan{$int[0]},,$int[3],$int[2]\n";
	    push( @intstatus, "$$devref{host},$int[0],$int[6],$vlan{$int[0]},,$int[3],$int[2]" );
	}
